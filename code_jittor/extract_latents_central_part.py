    ...
    for (i, (geo_input, origin_geo_input, logrmax, logrmin, smax, smin, filenames)) in enumerate(pbar):
        ...
        for j in range(geo_input.shape[0]):
            ...
            if os.path.exists(img_filename):
                ...
                # print('{} {} {} '.format(type(id_ts), type(id_bs), type(geo_zs[j:(j + 1), :])))
                
                data_dict = {'geo_z': geo_zs[j:(j + 1), :].data, 'id_ts': id_ts.data, 'id_bs': id_bs.data}
                sio.savemat(os.path.join(sub_dir, '{}_{}.mat'.format(model_id, args.central_part_name)), data_dict)
                if (dec.shape[1] == 4):
                    dec[:, 3, :, :] = (((dec[:, 3, :, :] > 0).float() - 0.5) * 2)
                # jt.misc.save_image(jt.misc.make_grid(dec), os.path.join(sub_dir, '{}_{}.png'.format(model_id, args.central_part_name)))
                # jt.misc.save_image(jt.misc.make_grid(patches), os.path.join(sub_dir, '{}_{}1.png'.format(model_id, args.central_part_name)))
                index += 1
...
if (__name__ == '__main__'):
    ...
    ckpt_dict = jt.load(args.geovae_ckpt)
