...
def extract_latents_patch(loader, model, geo_model, args):
    ...
    aug_transform = jt.transform.Compose([
                        jt.transform.Resize(((args.height * 3), (args.width * 4))), 
                        jt.transform.CenterCrop(((args.height * 3), (args.width * 4))), 
                        jt.transform.ColorJitter(brightness=0.3, contrast=0.3, saturation=0.3, hue=0.3), 
        ...
        for j in range(geo_input.shape[0]):
            ...
            if os.path.exists(img_filename):
                ...
                for k in range(20):
                    ...
                    data_dict = {'geo_z': geo_zs[j:(j + 1), :], 'id_ts': id_ts, 'id_bs': id_bs}
...
if (__name__ == '__main__'):
    ...
    ckpt = jt.load(args.vqvae_ckpt, map_location=args.device)
