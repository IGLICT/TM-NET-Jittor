    ...
    with jt.no_grad():
        ...
        for i in tqdm(range(size[0])):
            for j in range(size[1]):
                ...
                # sample = jt.multinomial(prob, 1).squeeze((- 1))
...
def get_central_vgg(vgg_model, filenames, args):
    ...
    all_central_vggs = jt.stack(all_central_vggs)
...
if (__name__ == '__main__'):
    ...
    dataset = LatentGeoVGG2LevelsDataset(args.path, mode='val', part_name=args.part_name)
    ...
    for (i, (geo_zs, top, bottom, central_vggs, ID)) in enumerate(ploader):
        ...
        top_condition = jt.contrib.concat([geo_zs, all_central_vggs], dim=3)
        print(top_condition.shape)
        for k in range(5):
            top_sample = sample_model(model_top, device, args.batch, [96, 16], args.temp, condition=top_condition)
            ...
            # merged_image = jt.zeros(args.batch, 4, 768, 1024).to(args.device)
            merged_image = jt.zeros((args.batch, 4, 768, 1024))
            H_begin = [0, 256, 256, 256, 256, 512]
            W_begin = [256, 0, 256, 512, 768, 256]
