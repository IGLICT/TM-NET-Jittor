...
def get_dataloader(config, mode):
    ...
    if module == 'vqvae':
        ...
        dataset = VQVAEDataset(
                            ...
                            height=height, 
                            width=width)
        dataloader = dataset.set_attrs(batch_size=batch_size, shuffle=is_shuffle, num_workers=num_workers, drop_last=True)
    elif module == 'spvae':
        dataset = SPVAEDataset(data_root, 
                            mode, )
        dataloader = dataset.set_attrs(batch_size=batch_size, shuffle=is_shuffle, num_workers=num_workers)
    elif module == 'geovae':
        part_name = config['data']['part_name']
