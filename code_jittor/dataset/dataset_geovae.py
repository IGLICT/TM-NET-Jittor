...
class GeoVAEDataset(dataset.Dataset):
    ...
    def normalize(self, logr_part, s_part):
        ...
        resultmin = (- 0.95)
        resultmax = 0.95
        logrmin = (logrmin - 1e-06)
        logrmax = (logrmax + 1e-06)
    ...
    def __getitem__(self, idx):
        ...
        return (np.float32(geo_input), np.float32(origin_geo_input), logrmax, logrmin, smax, smin, fullname)

if __name__ == '__main__':
    dataset = GeoVAEDataset(mat_dir='/mnt/f/wutong/data/table', mode='train', part_name='left_leg1')
    dataloader = dataset.set_attrs(batch_size=10, shuffle=True, num_workers=1)
