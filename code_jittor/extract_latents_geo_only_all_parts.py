...
def extract_latents_geo(loader, geo_models, args):
    ...
    for (i, (geo_inputs, origin_geo_inputs, logrmaxs, logrmins, smaxs, smins, filenames)) in enumerate(pbar):
        for j in range(geo_inputs.shape[0]):
            ...
            data_dict = {'geo_zs': geo_zs_all_parts[j:(j + 1), :].data}
...
if (__name__ == '__main__'):
    ...
    geo_dataset = GeometryAllPartsDataset(args.mat_dir, part_names=part_names, vertex_num=args.vertex_num, mode=args.mode)
    geo_loader = geo_dataset.set_attrs(batch_size=1, shuffle=False, num_workers=1)
    geo_models = []
    for (i, part_name) in enumerate(part_names):
        ...
        # if (i > 0):
        #     part_name = 'leg'
        geo_ckpt = os.path.join(args.geovae_ckpt_dir, part_name, 'latest.pkl')
